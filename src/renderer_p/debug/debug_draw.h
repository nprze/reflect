...
namespace rfct {
	...
	public:
		...
		static void flush(frameContext* ctx, RfctFrameSyncData& fd, vk::Framebuffer framebuffer, vk::RenderPass renderPass);
	private:
		debugDraw(vk::RenderPass renderPass, vk::Device device);
		void draw(frameContext* ctx, RfctSwapChain& swapChain, RfctFrameSyncData& fd, vk::Framebuffer framebuffer, vk::RenderPass renderPass);
