...
*/
	...
	vulkanDevice::vulkanDevice() :m_physicalDevice(chooseBestPhysicalDevice()), m_device(createDevice(m_physicalDevice)), m_queueManager(m_device.get(), m_physicalDevice), m_swapChain(m_device.get(), renderer::getRen().getWindow().createSurface(renderer::getRen().getInstance()), m_physicalDevice, renderer::getRen().getWindow().getExtent())
	{
