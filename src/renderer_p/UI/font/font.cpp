...
const rfct::RfctFontGlyph* rfct::RfctFont::getGlyph(char character) const {
	auto it = glyphMap.find(character);
	if (it == glyphMap.end()) RFCT_CRITICAL("trying to get a glyph which isn't in the font");
	return &it->second;
}

rfct::RfctFont::RfctFont(const std::string& path)
...
float rfct::RfctFont::getTextWidth(const std::string& text, float scale) {
    RFCT_PROFILE_FUNCTION();
	scale *= fontScale;
    float width = 0.f;
    const RfctFontGlyph* g;
    for (char c : text) {
        g = getGlyph(c);
...
float rfct::RfctFont::getFontHeight(float scale) {
    RFCT_PROFILE_FUNCTION();

    const RfctFontGlyph* g = getGlyph('g');
    const RfctFontGlyph* h = getGlyph('h');
