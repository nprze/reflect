...
namespace rfct {
	...
	public:
		...
		entity createStaticMesh(const std::string& path, glm::vec2 size, glm::vec2 pos, const glm::vec3& color, int cutoff = 4095); // loads mesh from .txt file (path should be pointing to a .txt). pos is left top coord.
